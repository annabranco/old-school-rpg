...
def simultaneous_attack(enemy):
		results_number_hero, results_text_hero = roll_dices(Hero['attack'], enemy['defense'], f'Attacking {enemy["name"]}')
		results_number_enemy, results_text_enemy = roll_dices(enemy['attack'], Hero['defense'], f'{enemy["name"]}\'s attack')

		if (results_number_hero <= 0 and results_number_enemy <= 0):
				mechanics_block()
				print_cinematics(f'You and {enemy["name"]} attack at the same time and your blows block each other.')
				initiative.initiative(enemy, 0)
		else:
				combat_mechanics.simultaneous_damage(results_number_hero, Hero, results_number_enemy, enemy)
